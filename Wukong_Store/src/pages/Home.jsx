...
// STATUS: IN PROGRESS
// ==========================================================================
// Home page. Hero banner + Popular Picks + New Arrivals. Products come from the Catalog.
...
// Product card for Popular Picks (click to open the detail page)
...
// Product card for New Arrivals (with a NEW badge)
...
export default function Home() {
  useTitle("Home");
  const navigate = useNavigate();
  const { products } = useCatalog();         // Read products from the catalog (database / sample data)
  const popular = products.slice(0, 3);      // First 3 items as popular picks
  const arrivals = [...products].reverse().slice(0, 4); // only 4; rest behind "View all"
