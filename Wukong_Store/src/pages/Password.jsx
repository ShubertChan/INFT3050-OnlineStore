...
// STATUS: IN PROGRESS
// ==========================================================================
// Update password page. Current / new / confirm password form. Demo only.
...
export default function Password() {
  ...
  // Save new password (demo: front-end length/match check only, no backend)
  const onSubmit = (event) => {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const password = String(data.get("password") || "");
    const confirm = String(data.get("confirm") || "");
    if (password.length < 8) { showToast("Password must be at least 8 characters"); return; } // At least 8 chars
    if (password !== confirm) { showToast("Passwords do not match"); return; }                 // Both must match
  ...
  return (
    <CustomerShell active="Home" accountActive={true}>
      ...
      <div className="password-layout">
        <AccountMenu active="Update Password" />{/* Left account menu */}
